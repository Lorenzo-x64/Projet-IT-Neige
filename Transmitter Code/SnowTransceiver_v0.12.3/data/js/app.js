...
(() => {
  ...
  const drawChart = () => {
    ...
    // Y-axis unit hint -- pinned to the top-left corner ABOVE the plot
    // area (textBaseline: bottom so the glyph sits above the y coordinate
    // and doesn't collide with the topmost tick number).
    ctx.fillStyle = "rgba(255,255,255,0.35)";
    ctx.textAlign = "left";
    ctx.textBaseline = "bottom";
    ctx.fillText(isImperial() ? "in" : "cm", 4, plot.y - 1);
  ...
  const REGION_CHANNEL_DEFAULT = { 0: 18, 1: 65 };  // EU=18, US=65
  ...
  const refreshLoraPage = () => {
    fetch("/api/settings", { cache: "no-store" })
      ...
      .then(s => {
        ...
        const ch = s.lora_channel ?? 18;
        if ($("#lora-ch")) {
          $("#lora-ch").value = ch;
          $("#lora-ch-hint").textContent = `ch ${ch} · ${channelToMhz(ch).toFixed(3)} MHz`;
        }
        setLoraSegActive('[data-lora-region]', s.lora_region ?? 0);
  ...
  const initLoraPage = () => {
    // Region segmented buttons -- snap channel to EU=18 / US=65 when picked
    $$('[data-lora-region]').forEach(btn => {
      btn.addEventListener("click", () => {
        const r = parseInt(btn.dataset.loraRegion, 10);
        setLoraSegActive('[data-lora-region]', r);
        const defaultCh = REGION_CHANNEL_DEFAULT[r] ?? 18;
        $("#lora-ch").value = defaultCh;
        $("#lora-ch-hint").textContent = `ch ${defaultCh} · ${channelToMhz(defaultCh).toFixed(3)} MHz`;
        markLoraDirty();
      });
    });
    ...
    // Channel slider live-updates the hint label
    if ($("#lora-ch")) {
      $("#lora-ch").addEventListener("input", () => {
        const v = $("#lora-ch").value | 0;
        $("#lora-ch-hint").textContent = `ch ${v} · ${channelToMhz(v).toFixed(3)} MHz`;
        markLoraDirty();
      });
    }
    ...
    if ($("#lora-apply")) {
      $("#lora-apply").addEventListener("click", () => {
        ...
        const body = {
          ...
          lora_region:       parseInt(region, 10),
          lora_channel:      $("#lora-ch").value | 0,
        ...
        fetch("/api/settings", {
          ...
          .then(() => {
            ...
            loraFormDirty = false;
            // Re-fetch to verify clamped values
  ...
  const startFastUltrasonicLoop = () => {
    ...
    const tick = async () => {
      try {
        ...
        lastDistText  = setReadout("hero-distance", u.distance_cm, lastDistText);
        lastDepthText = setReadout("hero-depth",    u.depth_cm,    lastDepthText);

        const heroFresh = document.getElementById("hero-fresh");
        if (heroFresh) heroFresh.textContent = u.fresh ? "Live" : "Stale";

        const snowDistEl  = document.getElementById("snow-distance");
        if (snowDistEl)  snowDistEl.textContent  = fmtCm(u.distance_cm);
        const snowDepthEl = document.getElementById("snow-depth");
        if (snowDepthEl) snowDepthEl.textContent = fmtCm(u.depth_cm);
      } catch (e) {
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    $("#btn-tare-reset").addEventListener("click", async () => {
      try {
        await api("/api/tare/reset", { method: "POST" });
