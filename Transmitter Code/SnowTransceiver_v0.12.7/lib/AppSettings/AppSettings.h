...
public:
    ...
    // Temperature compensation for the ultrasonic sensor.
    // Speed of sound depends on air temperature: v(T) = 331.3 + 0.606*T (m/s).
    // When enabled (and AHT temperature reading is fresh), distanceCm() is
    // scaled by v(T)/v(20 deg C) so the value matches reality. Default OFF
    // for backwards-compatible behaviour.
    bool   tempCorrectionEnabled() const { return _tempCorrection; }
    void   setTempCorrectionEnabled(bool on);
...
private:
    ...
    bool     _tempCorrection = false;     // ultrasonic temperature compensation
